...
interface Props {
  ...
  setTotalSize: Dispatch<SetStateAction<any>>;
  ...
  const buildItem = () => {
    ...
    for (let key in navLinkData) {
      if (navLinkData[key]['url'] !== undefined) {
        ...
        items.push(
          getItem(
            <div className={styles.header}>{navLinkData[key]['name']}</div>,
            key,
          ),
        );
        ...
        for (let newKey in navLinkData[key]) {
          if (flag) {
            ...
            firstParam = (
              <div
                onClick={() => handleHeaderClick(newKey)}
                className={styles.header}
              >
                <div>{navLinkData[key][newKey]['name']}</div>
              </div>
            );
            flag = !flag;
            continue;
