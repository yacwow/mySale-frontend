...
const App: React.FC<Props> = (props) => {
    ...
    const build = () => {
        return dataList.map((item, index) => {
            return (
                <div key={index} className={styles.list}><FullyParamPicture 
                    imgSrc={item.imgSrc}
                    href={item.href}
    ...
    const handleChange = (e: number) => {//翻页点击事件
        setPage(e)
    }
    return (
        <div className={styles.container}>
            <h3 className={styles.title}>
                ...
                <ul className={`${styles.titleRight} generalProductRightDisplay`}>
                    <li onClick={(e) => handleClick(e)} className={styles.active} value={'recommend'}>並び順</li>
                    <li onClick={(e) => handleClick(e)} value={'newProduct'} >新着順</li>
                    <li onClick={(e) => handleClickO(e)} value={'asc'} >安い順
                        <CaretUpOutlined /></li>
                    <li onClick={(e) => handleClickO(e)} value={'desc'} >高い順
                        <CaretDownOutlined /></li>
                </ul>
            </h3>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Pagination hideOnSinglePage total={size} pageSize={60} current={page} onChange={handleChange} showSizeChanger={false} />
            </div>
        </div>
    )
};
