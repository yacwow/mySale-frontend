...
const App: React.FC = () => {
  ...
  const [itemRankingDataProp, SetItemRankingDataProp] =
  ...
  useEffect(() => {
    ...
    SetItemRankingDataProp(
      dataList?.[selectItem].map((item, index) => {
        return (
          <PictureWithModel
            key={index}
            content={{ title: item.productDescription, price: item.price }}
            rank={item.rank}
  ...
  return (
    <div className={styles.container}>
      ...
      />
      {itemRankingDataProp ? (
        <ItemRankingLayout itemRankingDataProp={itemRankingDataProp} />
      ) : null}
      <div className={styles.center} style={{ paddingTop: 30 }}>
        <NavLink
          to={`/generalSale/${firstLevelCategory}/${secondLevelCategory}`}
