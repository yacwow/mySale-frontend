...
const App: React.FC = () => {
  ...
  const { secondHalfPriceExpireTime } = useModel('global');
  useEffect(() => {
    if (page === 1) {
  ...
  const buildShowingPicture = () => {
    return pictureData.map((item: any, index: number) => {
      return (
  ...
  return (
    <div
      style={{ margin: '0 auto', width: 1200, marginBottom: 30, marginTop: 60 }}
    >
      {secondHalfPriceExpireTime && (
        <TimeCount expire={secondHalfPriceExpireTime} />
      )}
      <div
