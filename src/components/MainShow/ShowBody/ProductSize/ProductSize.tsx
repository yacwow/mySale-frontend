...
const App: React.FC<Props> = (props) => {
    ...
    const buildProductSizeTable = () => {
        ...
        const thList=thdata.map((item,index)=>{
            return <th key={index} style={{ width: 100 }}>{item}</th>
        })
        let tbodyList = []
        for(let i=1;i<tableValue.length;i++){
            ...
            let tr = <tr key={i} className={i % 2 === 0 ? styles.coloredBG : styles.whiteBG}>{tdList}</tr>;
            tbodyList.push(tr)
        }
    ...
    return (
        <div className={styles.container}>
            <div className={styles.wrapper}>
                {buildProductSizeTable()}
            </div>

            <div className={styles.material}>
                {buildProductMaterialTable()}
            </div>
        </div>
