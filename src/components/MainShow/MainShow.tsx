...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // console.log(params)
    request(`/api/salesPageProduct/${params}`, { params: { userEmail } }).then((data) => {
      if (data.result) {
        ...
        if (salesPageProduct === undefined) { history.push("/bestseller"); return }
        else {
          ...
          let bread = {
            first: { href: `/generalsale?firstLevel=${firstLevelCategory}`, content: firstLevelCategory }
            , second: { href: `/generalsale?firstLevel=${firstLevelCategory}&secondLevel=${secondLevelCategory}`, content: secondLevelCategory },
          ...
          for (let i = 0; i < salesPagePictures.length; i++) {
            headerPictures[salesPagePictures[i].color] = salesPagePictures[i].bigimgsrc;
          }
          let sellerDetailPictures = [];
          // eslint-disable-next-line guard-for-in
          for (const key in headerPictures) {
            sellerDetailPictures.push({ color: key, imgSrc: headerPictures[key] })
          }
          setSellerDetailPicture(sellerDetailPictures);
          // console.log(salesPageProduct)
          let sortedPictureData = salesPageProduct.salesPagePictures.sort((a, b) => {
  ...
  return (
    <div >
      {breadList ? <ShowHeader breadList={breadList} productHref={productHref} wishList={wishList}
        showComponent={showComponent} secondOneHalf={secondOneHalf} discount={discount} timesale={timesale}
        sellerDetailPicture={sellerDetailPicture} sellDetailData={sellDetailData} /> : null}
      <div className={styles.main}>
        {showBodyPictureData ? <ShowBody showBodyPictureData={showBodyPictureData} tableValue={tableValue}
          productMaterial={productMaterial} reviewContext={reviewContext} /> : null}
        {leftSidePromotionLineList ? <LeftSidePromotionLine leftSidePromotionLineList={leftSidePromotionLineList} /> : null}
      </div>
