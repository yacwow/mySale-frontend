...
const App: React.FC<Props> = (props) => {
  ...
  const [colorSelectPicture, setColorSelectPicture] = useState(
    pictureSrc[0].color,
  ); //颜色选择，后续会有变动，和左边联动
  const [sizeSelectOption, setSizeSelectOption] = useState(
    sellDetailData.sizeList[0],
  ); //大小选择
  ...
  useEffect(() => {
    if (wishList.isLogin) {
      ...
      if (wishListStr) {
        ...
        if (
          Array.isArray(newWishListStr) &&
          newWishListStr.includes(sellDetailData.productId)
        ) {
          setWishlist('/heart.png');
        }
      }
    ...
    if (!isLogin) {
      ...
      if (productList) {
        ...
        for (let i = 0; i < newProduct.length; i++) {
          if (
            newProduct[i].productId === sellDetailData.productId &&
            newProduct[i].size === sizeSelectOption &&
            newProduct[i].color === colorSelectPicture
          ) {
  ...
  const handleAddToWishlist = () => {
    ...
    if (!isLogin) {
      ...
      if (wishlist === '/heart1.png') {
        ...
        request(`/api/secure/updateWishListAfterLogin`, {
          params: { productId: sellDetailData.productId, delete: false },
        }).then((data) => {
          if (data.result) {
          }
        });
        ...
        setWishListNum(wishListNum - 1);
        request(`/api/secure/updateWishListAfterLogin`, {
          params: { productId: sellDetailData.productId, delete: true },
        }).then((data) => {
          if (data.result) {
          }
        });
      ...
      return (
        <label className={styles.radioInline} key={index}>
          <input
            type="radio"
            onClick={() => {
              handleColorSelectPicture(item.color, index);
            }}
            checked={colorSelectPicture === item.color ? true : false}
            className={styles.skuColor}
            value={item.color}
            readOnly
          />
          <span>
      ...
      return (
        <label className={styles.radioInline} key={index}>
          <input
            ...
            checked={sizeSelectOption === item ? true : false}
            className={styles.skuSize}
          />
          <span>
            {' '}
            <b
              className={styles.text}
              style={sizeSelectOption === item ? { background: '#E3BA84' } : {}}
            >
              {item}
            </b>{' '}
          </span>
  ...
  return (
    <div className={styles.container}>
      ...
      <a
        href={sellDetailData.bestSellHref}
        className={styles.bestSellHref}
        style={{
          marginTop: '10px',
          marginBottom: '6px',
      ...
      >
        <span style={{ color: 'orange' }}>
          数量限定タイムセール開催中、こちらでチェック
        </span>
      </a>
      <p className={styles.oldPrice}>
        <del>
          参考価格：<span>￥{NumToString(sellDetailData.oldPrice)} 税込</span>
        </del>
      </p>
      <p className={styles.price}>
        販売価格：<del>￥{NumToString(sellDetailData.price)} 税込</del>
      </p>
      <p className={styles.restrict} style={{ color: '#B52929' }}>
        販売価格：
        <span className={styles.variatePrice} style={{ color: ' #B52929' }}>
          ￥{NumToString(sellDetailData.newPrice)}
        </span>{' '}
        <b>税込</b>
      </p>
      <p style={{ paddingBottom: 30 }}>
        <span
          style={{ height: 25, float: 'left', color: 'black', fontSize: 10 }}
        >
          ポイント :5倍{Math.floor(sellDetailData.newPrice / 20)}{' '}
        </span>
        <img
          src="/norton.png"
      ...
      <div className={styles.productId}>
        <h2>
          商品番号：<span>{sellDetailData.productId}</span>
        </h2>
        ...
        {/* 要点击到另一个界面 */}
        <a className={styles.reviewContext}>
          レビュー / <b>{sellDetailData.comment}</b> 件
        </a>
      </div>
      <div className={styles.optionElement}>
        <div className={`${styles.changeBox} pictureColorSelect`}>
          <h2>
            {' '}
            カラー :{' '}
            <span className={styles.changeText}>{colorSelectPicture}</span>
          </h2>
        ...
        <div className={styles.changeBox}>
          <h2>
            {' '}
            サイズ : <span className={styles.changeText}></span>
          </h2>
        ...
        <div id="btn-wish">
          <Button className={styles.Btn2} onClick={handleAddToWishlist}>
            ...
            />
            お気に入りに追加する
            <span style={{ paddingLeft: 10 }}>{wishListNum}</span>
          </Button>
