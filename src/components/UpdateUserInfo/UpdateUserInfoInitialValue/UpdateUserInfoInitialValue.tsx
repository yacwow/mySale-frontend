...
const App: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    ...
    request(`/api/getAddressList/${initialValues.city}`).then((data) => {
      setAreaStatus(false);
      // console.log(data);
      setAreaData(data.data.areaList);
    });
  }, []);
  //模态框关闭
  ...
  const handleBlur = (e: any) => {
    request(`/api/getAddress/${e.target.value.trim()}`).then(
      (serverSideData) => {
        ...
        setCityData(data.cityList);
        setAreaData(data.areaList);
        setCityStatus(false);
        setAreaStatus(false);
        setPostStatus(true);
      },
  ...
  const handleFieldsChange = (changedFields: any, allFields: any) => {
    ...
    if (changedFields[0].name[0] === 'province') {
      ...
      console.log(allFields[8].value, allFields, allFields[10]);
      let province, city, area;
      for (let i = 0; i < allFields.length; i++) {
        if (allFields[i].name.includes('province')) {
          province = allFields[i].value;
        } else if (allFields[i].name.includes('city')) {
          city = allFields[i].value;
        } else if (allFields[i].name.includes('area')) {
          area = allFields[i].value;
        }
      }
      console.log(province, city, area);
      request(`/api/getPostCode/${province}/${city}/${area}`).then((data) => {
        if (data.result) {
  ...
  const onFinish = (values: any) => {
    ...
    params.invoiceId = initialValues.invoiceId ? initialValues.invoiceId : -1; //有这个鬼东西就是更新，没有就是添加，这其实是地址的id
    setParams(params);
    setOpenUpdate(true);
  };
  const handleUpdate = () => {
    //后台给个需要update/add
    request('/api/secure/needUpdateOrAdd', { params: params }).then((data) => {
      if (data.result) {
      } else {
        //message告知有错误，等下重新修改
      }
    ...
    <>
      ...
      >
        ...
        <div>
          ...
          <Form.Item style={{ width: 810, marginLeft: 270 }} name="postcode">
            <div>
              <a
                style={{
                  background: '#c9302c',
        ...
        <Form.Item
          ...
          name="city"
          rules={[
            { required: true, message: '市をお願いします。', whitespace: true },
          ]}
          label={
            <span
              style={{
        ...
        <Form.Item
          ...
          name="area"
          rules={[
            { required: true, message: '町をお願いします。', whitespace: true },
          ]}
        >
          {
            <Select
        ...
        >
          ...
          >
            <Button
              ...
              style={{ marginRight: 10, background: '#bb8a4f' }}
            >
              修正放棄確定です
            </Button>
            <Button
      ...
      <Modal centered open={openUpdate} closable={false} footer={null}>
        <div
          style={{
            textAlign: 'center',
            fontSize: 16,
