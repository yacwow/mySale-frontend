...
const App: React.FC<Props> = (props) => {
        ...
        return weekPromotionBannerData.map((item, index: number) => {
            ...
            className={styles.wrapper}>
                <div className={styles.rankingImg}>
                    ...
                    <span
                        className={styles.rankingNum}>{index + 1}</span>
                </div>
                <NavLink to={item.href}>
                    <div style={{ position: 'relative' }} className={styles.picture}>
                        <LazyLoadImage 
                            ...
                            alt="" />
                        {item.discount ?
                            <LazyLoadImage src='img/discount.jpg' style={{
                                position: 'absolute',
                                bottom: '5px', right: '5px', width: '30%'
                            }} />
                            : null}
                    </div></NavLink>

                <div  className={styles.item}>{item.price} <span
                    style={{ fontSize: 14, color: ' #000' }}>(税込)</span></div>
            </div>)
        })
    }
    ...
    return (
        <div >
            <div style={{ textAlign: 'center', margin: '40px 0', fontSize: 24, fontWeight: 600 }}>今週のベストセラーTOP20</div>
