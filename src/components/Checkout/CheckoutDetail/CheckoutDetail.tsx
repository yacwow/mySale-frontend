...
const App: React.FC<Props> = (props) => {
  ...
  const buildTrList = () => {
    return data.map((item, index: number) => {
      return (
        <tr key={item.productId + index} style={{ height: 120 }}>
          <td>
            <a title={item.title} href={item.href}>
              <img width={59} alt={item.title} src={item.imgSrc} />
            </a>
          </td>
          <td>
            <a href={item.href}>
              <div className={styles.itemData}>
                <h2 style={{ margin: 0, padding: 0 }}>
                  ...
                  <p className={styles.title}>{item.productDescription}</p>
                </h2>
          ...
          <td className={styles.moveWishList} style={{ width: 125 }}>
            <span className={styles.cartPrice}>
              <span>
                ￥{NumToString(item.amount * item.price)}
                <span
                  style={{
                    fontSize: 13,
                    color: ' #7d7d7d',
        ...
        surFix={
          <div style={{ cursor: 'pointer' }} onClick={showModal}>
            ...
            >
              ...
              <div>
                *商品配送予定日：通常商品は2～7営業日以内ご発送させていただきます。
                現時期コロナウイルスの影響により、配送は少し遅延が発生する場合
                もございます。ご理解の程よろしくお願いします。
              </div>
            </Modal>
          </div>
        }
      />
      <table className={styles.body} id="shopping-cart-table">
        <thead>
          <tr>
            <th rowSpan={1}></th>
            <th rowSpan={1}>
          ...
          <div className={styles.left}>
            <div className={styles.discount}>
              ...
              <input
                ...
                type="text"
                placeholder="メッセージを枠内にご記入ください。"
              />
            </div>
            <div style={{ padding: '20px 0 10px', fontSize: 12 }}>
              <NavLink
                to="/companyIntroduce?9"
                style={{ color: '#b44b2e', textDecoration: 'underline' }}
              >
                ※キャンセル/返品/支払い戻しに関する事項
              </NavLink>
            </div>
          </div>
                  ...
                  <tbody>
                    ...
                    <tr className={styles.topLine}>
                      ...
                      <td className={styles.discount}>
                        ￥{NumToString(total.total)}(税込)
                      </td>
                    </tr>
                    {total.secondHalfDiscount ? (
                      <tr>
                        <th>半額セール割引：</th>
                        <td className={styles.discount}>
                          -￥{NumToString(total.secondHalfDiscount)}
                        </td>
                      </tr>
                    ) : null}
                    {total.timelyDiscount ? (
                      <tr>
                        <th>クーポン割引</th>
                        <td className={styles.discount}>
                          -￥{NumToString(total.timelyDiscount)}
                        </td>
                      </tr>
                    ) : null}
                    {total.discount ? (
