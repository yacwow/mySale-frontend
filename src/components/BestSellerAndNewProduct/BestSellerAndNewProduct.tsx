...
const App: React.FC = () => {
  const [selectItem, setSelectItem] = useState('dailyNew');
  const [bestSellerAndNewProductDataList, setBestSellerAndNewProductDataList] =
    useState<dataListType>();
  useEffect(() => {
  ...
  return (
    <div className={styles.container}>
      <BestSellerAndNewProductHeader
        selectItem={selectItem}
        setSelectItem={setSelectItem}
        dataList={BestSellerAndNewProduct}
      />
      <div className={styles.pictureWrapper} style={{ width: 1500 }}>
        {bestSellerAndNewProductDataList !== undefined &&
        bestSellerAndNewProductDataList[selectItem] ? (
          <BestSellerAndNewProductBanner
            dots={false}
            slidesToShow={5}
            newsBanner={bestSellerAndNewProductDataList[selectItem]}
            arrows={false}
          />
        ) : null}
      </div>
