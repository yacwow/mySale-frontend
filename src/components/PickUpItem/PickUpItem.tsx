...
interface pickupDataType{
    href:string,expireTime:number;intro:string,rank:number;imgSrc:string,
}
const App: React.FC = () => {
    ...
    const buildLiList = () => {
        return liList.map((item, index) => {
            return (<li key={index} className={styles.topLine}>
                <PickUpPicture date={ formatTime(item.expireTime) } imgSrc={item.imgSrc} href={item.href} intro={item.intro}></PickUpPicture>
            </li>)
        })
